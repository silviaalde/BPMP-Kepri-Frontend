...
const Page = () => {
        ...
        },
        {
            ...
            renderCell: (params : GridRenderCellParams) => (
                <div className="flex gap-2 items-center h-full">
                    <Link href={`/admin/webinar/update/${params.row.id}`}>
                        <button 
    ...
    const fetchData = async () => {
        try {
            const response = await Webinar.getWebinar();
    ...
    const handleDelete = async (id : string) => {
        try {
            const response = await Webinar.dropWebinar(token??"", id);
