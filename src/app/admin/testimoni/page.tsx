...
const Page = () => {
        ...
        },
        {
            ...
            renderCell: (params : GridRenderCellParams) => (
                <div className="flex gap-2 items-center h-full">
                    <Link href={`/admin/testimoni/update/${params.row.id}`}>
                        <button 
    ...
    const fetchData = async () => {
        try {
            const response = await Testimoni.getTestimoni();
    ...
    const handleDelete = async (id : string) => {
        try {
            const response = await Testimoni.dropTestimoni(token??"", id);
